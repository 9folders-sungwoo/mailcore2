...
build_git_ios()
{
  ...
  devicearchs="arm64"
  sdkminversion="13.0"
  sdkversion="`xcodebuild -showsdks 2>/dev/null | grep 'sdk iphoneos' | sed 's/.*iphoneos\(.*\)/\1/'`"
...
#        -output "$name-$version/$name/lib/$library"
	mv "Release-iphoneos/$library" "$name-$version/$name/lib/iphoneos/$library"
	mv "Release-iphonesimulator/$library" "$name-$version/$name/lib/iphonesimulator/$library"
